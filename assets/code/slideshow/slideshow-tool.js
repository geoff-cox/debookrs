function waitForCanvas(callback) {
    ...
    const check = () => {
      ...
      canvases.forEach(canvas => {
        ...
        const idRaw = canvas.id;
        if (!idRaw.includes("|")) return;
  
        const [id, configPath] = idRaw.split("|");
  
        if (id && configPath) {
          canvas.id = id; // update the ID to remove config portion
          canvas.dataset.loaded = "true"; // prevent duplicate init
          console.log(`[Slideshow] Initializing ${id} with ${configPath}`);
          callback(id, configPath);
        }
      });
      ...
      if (waited < maxWait) {
        ...
        console.error("[Slideshow] Canvas not found after waiting.");
      }
    };
...
//   waitForCanvas((canvasId) => {
//     const script = document.querySelector("script[src$='slideshow-tool.js']");
//     const basePath = script.src.split("/").slice(0, -1).join("/");
//     const configPath = `${basePath}/slideshow-config.json`;
...
//   });
  ...
  waitForCanvas((canvasId, configPath) => {
    fetch(configPath)
      .then(res => res.json())
      .then(config => runSlideshow(canvasId, config))
      .catch(err => console.error(`[Slideshow] Failed to load ${configPath}:`, err));
  });
  ...
  function runSlideshow(canvasId, config) {
    ...
    canvas.addEventListener("mousemove", (event) => {
      ...
      hoverBack = x >= back.x && x <= back.x + buttonWidth &&
                  y >= back.y && y <= back.y + buttonHeight &&
                  currentSlide > 0;
  
      hoverNext = x >= next.x && x <= next.x + buttonWidth &&
                  y >= next.y && y <= next.y + buttonHeight &&
