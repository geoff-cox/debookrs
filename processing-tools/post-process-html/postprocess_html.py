    ...
    try:
        ...
        for i, item in enumerate(data):
            ...
            if not isinstance(item, dict):
                raise ValueError(f"Item {item_index} in JSON is not a dictionary.")
            ...
            if not isinstance(find, str) or not isinstance(replace, str):
                raise ValueError(
                    f"Item {item_index} must contain string 'find' and 'replace' fields."
                )
            if not isinstance(use_regex, bool):
                raise ValueError(f"Item {item_index} has non-boolean 'regex' field.")
            ...
            if use_regex:
                try:
                    ...
                    raise ValueError(f"Invalid regex in item {item_index}: {exc}") from exc
