...
<!-- AUDIT-STATUS:BEGIN (generated by processing-tools/audit-status/audit_status.py — do not edit by hand) -->

## Task status

_Generated by `processing-tools/audit-status/audit_status.py` against `origin/main`. Re-run it to refresh._

| Task | Status | Merged as | Title |
|------|--------|-----------|-------|
| H1 | ✅ done | #187 | Fix the confirmed math errors in worked examples, answer keys, an… |
| H2 | ✅ done | #189 | Rebuild the partial-fractions appendix section (`aa-bookends/a1-a… |
| H3 | ✅ done | #190 | Create an automated CAS verification pass for all worked mathematics |
| H4 | ✅ done | #188 | Fix repo/book metadata |
| H5 | ✅ done | #197 | Finish the chapter 13 eigenvalue story |
| H6 | ✅ done | #198 | Add existence & uniqueness treatment |
| H7 | ✅ done | #191 | Add a "lost solutions" callout to Separation of Variables |
| H8 | ✅ done | #196, #192 | Backfill missing/empty answers and solutions; fix statement/solut… |
| H9 | ✅ done | #201 | Add cumulative mixed-method reviews + a "which method?" selector |
| H10 | ✅ done | #202 | De-duplicate exercise pools and re-tier difficulty |
| H11 | ✅ done | #199 | Book-wide notation standardization |
| H12 | ✅ done | #200 | Markup/structure standardization pass |
| H13 | ✅ done | #194 | Copy-edit sweep (~80 catalogued issues) |
| H14 | ✅ done | #204 | Finish the quick-reference appendices |
| H15 | ✅ done | #205 | Populate or drop the empty back-matter apparatus |
| H16 | ✅ done | #207 | Add alt-text/descriptions to all images and interactives |
| H17 | ✅ done | #206 | Complete reading checkpoints in uncovered sections |
| M1 | ✅ done | #208 | Sweep build-integrity defects (Rec 4) |
| M2 | ✅ done | #209 | Prune orphaned assets and generated artifacts (Rec 7) |
| M3 | ⬜ pending | — | Build out Chapter 14 — Nonlinear Systems (Rec 9) |
| M4 | ⬜ pending | — | Re-enable the 15 "Modeling with…" sections as a Modeling & Projec… |
| M5 | ⬜ pending | — | Add Euler error analysis and an Improved Euler/Heun preview (Rec 14) |
| M6 | ⬜ pending | — | Add or preview other standard first-order methods (Rec 15) |
| M7 | ⬜ pending | — | Add variation of parameters and spring-mass applications to the s… |
| M8 | ⬜ pending | — | Expand dynamic/randomized exercises (Rec 22) |
| M9 | ⬜ pending | — | Add applied problems to every method chapter's exercise set (Rec 24) |
| M10 | ⬜ pending | — | Complete audio narration for chapters 7–14 (Rec 32) |
| L1 | ✅ done | #195 | Add validation to PR CI (Rec 5) |
| L2 | ⬜ pending | — | Archive/remove stranded master-file variants (Rec 6) |
| L3 | ⬜ pending | — | Add convolution and Dirac delta/impulse to the Laplace unit (Rec 17) |
| L4 | ⬜ pending | — | Per-chapter assessment floor (Rec 19) |
| L5 | ⬜ pending | — | Decide the print/PDF strategy (Rec 31) |
| L6 | ⬜ pending | — | Glossary flashcards export (Rec 36) |

**High:** 17/17 done &nbsp;·&nbsp; **Medium:** 2/10 done &nbsp;·&nbsp; **Low:** 1/6 done &nbsp;·&nbsp; **Overall:** 20/33.
...
Nearly every confirmed error in Appendix A is mechanically detectable: re-derive each worked answer with SymPy and diff against the printed `<answer>`/`<solution>`. Build it as a script in `processing-tools/` (extract `<md>` math from solution blocks, verify DE solutions by substitution, verify PFD/transform tables symbolically) and keep it as a repeatable regression gate for future edits. This also covers residual risk in chapters 8–12, whose audit passes were the lightest (see Caveats).
