...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(configPath)
    ...
    .then(config => {
      ...
      const observer = new MutationObserver(() => drawSlide());
...
function runSlideshow(canvasId, config) {
  ...
  const controlPanelWidth = settings.controlPanelWidth || 300;
  ...
  function drawHorizontalLayout(ctx, img, textColor, backgroundColor, buttonColor, hoverColor, borderColor, indicatorInactive) {
    ...
    drawTextBox(panelX + canvasPadding, panelY + canvasPadding, controlPanelWidth - 2 * canvasPadding, panelHeight, textColor, backgroundColor, borderColor);
    drawButtons(panelX + canvasPadding, panelY + panelHeight - buttonHeight - 30, buttonColor, hoverColor);
    drawSlideIndicators(panelX + canvasPadding, panelY + panelHeight - 10, controlPanelWidth - 2 * canvasPadding, buttonColor, indicatorInactive);
  }

  function drawVerticalLayout(ctx, img, textColor, backgroundColor, buttonColor, hoverColor, borderColor, indicatorInactive) {
    const imageHeight = settings.imageHeight || canvas.height * 0.65;
    ...
    drawTextBox(canvasPadding + 10, panelY + 10, canvas.width - 2 * canvasPadding - 20, panelHeight - buttonHeight - 40, textColor, backgroundColor, borderColor);
    drawButtons((canvas.width - 2 * buttonWidth - buttonPadding) / 2, panelY + panelHeight - buttonHeight - 20, buttonColor, hoverColor);
    drawSlideIndicators(canvas.width / 2, panelY + panelHeight - 5, null, buttonColor, indicatorInactive, true);
  }
