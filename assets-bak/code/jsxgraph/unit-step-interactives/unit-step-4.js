...
// === Initialize Boards ===
const titleBoard = JXG.JSXGraph.initBoard(boardId + "-title", {
...
const plotBoard = JXG.JSXGraph.initBoard(boardId + "-plots", {
...
class Part {
	constructor(parent, func, xStart, xEnd, options = {}) {
		this.parent = parent;
		this.func = func;
		this.xStart = xStart;
		this.xEnd = xEnd;
		...
		this.curve = plotBoard.create('functiongraph', [
			x => this.func(x),
			() => this.xStart,
			() => this.xEnd
		], {
			strokeColor: options.strokeColor || parent.color,
		...
		const dotOpts = (type) => ({
			...
			fixed: true,
			size: 3,
			color: options.strokeColor || parent.color,
			strokeColor: options.strokeColor || parent.color,
			fillOpacity: type === 'open' ? 0 : 1,
			highlight: false,
			label: { visible: false }
		});

		if (options.leftDot) {
			this.leftPoint = plotBoard.create('point', [() => this.xStart, () => this.func(this.xStart)], dotOpts(options.leftDot));
		}
		if (options.rightDot) {
			this.rightPoint = plotBoard.create('point', [() => this.xEnd, () => this.func(this.xEnd)], dotOpts(options.rightDot));
		}
	}

	setVisibility(visible) {
		this.curve.setAttribute({ visible });
		if (this.leftPoint) this.leftPoint.setAttribute({ visible });
		if (this.rightPoint) this.rightPoint.setAttribute({ visible });
	}

	setOpacity(opacity) {
		this.curve.setAttribute({ opacity });
		if (this.leftPoint) this.leftPoint.setAttribute({ opacity });
		if (this.rightPoint) this.rightPoint.setAttribute({ opacity });
	}
}
...
class GraphObject {
	...
	setOpacity(alpha) {
		...
		this.jumpMarkers.forEach(({ open, closed }) => {
			open.setAttribute({ opacity: alpha, visible: this.selected });
			closed.setAttribute({ opacity: alpha, visible: this.selected });
		});
		this.parts.forEach(part => part.setOpacity(alpha));
	}

	addPart(func, xStart, xEnd, options = {}) {
		const part = new Part(this, func, xStart, xEnd, options);
...
function checkAndShowProduct() {
	...
	Object.values(curves).forEach(curve => {
		...
		curve.parts.forEach(part => part.setVisibility(!productOn));
	});
}
