...
3. Add each `<idx>` tag inside a parent element that supports index tags.
4. If a concept appears only in a location that cannot safely contain `<idx>`, flag it in notes instead of forcing the tag.
5. Avoid duplicate or overly dense index entries.
6. Keep index text concise and useful to readers.
