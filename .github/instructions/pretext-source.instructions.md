...
- Use `<m>` for inline math and `<md>` for displayed math.
...
- **Checkpoint emoji cues**: a reading-question bundle `<exercise>` title carries `🤔💭`; each individual checkpoint `<exercise>`/`<task>` title carries `📖❓`; "Recall:" flashback items use `↩️☝`; 👀 marks quick-review asides. Every student-facing checkpoint must have a title with one of these cues.
