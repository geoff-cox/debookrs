...
**Phases 3, 6 and 8D are finished, `<paragraphs>`-in-a-`<solution>` is cleared, every `*-model.ptx` is clean, `<audio>` is 0 book-wide, and the attribute-value screen is clean.** Regenerated at `c4f491a`, the largest remaining classes are `<solution>`/`<answer>` placement (**20** messages from 15 sites, 13 of them blocked or decided — see below), `<line>` used for line breaks (14, in the six section files never examined — see the R8 section), `<proof>` placement (12), the `<assemblage>` panel below (11), then `<p>`, `<interactive>`, `<md>` and `<statement>` at 8 apiece and `<tabular>` at 7. **161 messages now sit in 47 files.**
...
⚠️ **"Phase 1 dominates at 71%" was true at baseline and is no longer true.** At `c4f491a` the schema half is 161 across 47 files, and the largest class is `<line>` used for line breaks at 14. The ordering logic below still explains why the sweep ran in this order; it is no longer a statement about where the remaining work is.
